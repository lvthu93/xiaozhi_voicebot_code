...
final class Http2Reader implements Closeable {
    ...
    public static final class ContinuationSource implements jb {
        byte flags;
        int left;
        int length;
        short padding;
        ...
        public ContinuationSource(cm cmVar) {
            this.source = cmVar;
        }
        ...
        public long read(ck ckVar, long j) throws IOException {
            while (true) {
                ...
                if (i == 0) {
                    ...
                    long read = this.source.read(ckVar, Math.min(j, (long) i));
                    if (read == -1) {
                        return -1;
                    }
    ...
    public interface Handler {
        ...
        void alternateService(int i, String str, cq cqVar, String str2, int i2, long j);

        void data(boolean z, int i, cm cmVar, int i2) throws IOException;

        void goAway(int i, ErrorCode errorCode, cq cqVar);
    ...
    public Http2Reader(cm cmVar, boolean z) {
        this.source = cmVar;
        this.client = z;
        ContinuationSource continuationSource = new ContinuationSource(cmVar);
    ...
    private void readGoAway(Handler handler, int i, byte b, int i2) throws IOException {
        if (i < 8) {
            ...
            if (fromHttp2 != null) {
                cq cqVar = cq.i;
                if (i3 > 0) {
                    cqVar = this.source.b((long) i3);
                }
                handler.goAway(readInt, fromHttp2, cqVar);
                return;
            }
    ...
    public static int readMedium(cm cmVar) throws IOException {
        return (cmVar.readByte() & 255) | ((cmVar.readByte() & 255) << 16) | ((cmVar.readByte() & 255) << 8);
    }
    ...
    public void readConnectionPreface(Handler handler) throws IOException {
        if (!this.client) {
            cm cmVar = this.source;
            cq cqVar = Http2.CONNECTION_PREFACE;
            cq b = cmVar.b((long) cqVar.t());
            Logger logger2 = logger;
            if (logger2.isLoggable(Level.FINE)) {
                logger2.fine(Util.format("<< CONNECTION %s", b.o()));
            }
            if (!cqVar.equals(b)) {
                throw Http2.ioException("Expected a connection header but was %s", b.x());
            }
