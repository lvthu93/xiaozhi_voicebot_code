...
public final class Http2Codec implements HttpCodec {
    ...
    public class StreamFinishingSource extends f3 {
        long bytesRead = 0;
        boolean completed = false;

        public StreamFinishingSource(jb jbVar) {
            super(jbVar);
        }
        ...
        public long read(ck ckVar, long j) throws IOException {
            try {
                long read = delegate().read(ckVar, j);
                if (read > 0) {
                    this.bytesRead += read;
                }
                return read;
    ...
    public static List<Header> http2HeadersList(Request request) {
        ...
        for (int i = 0; i < size; i++) {
            cq m = cq.m(headers.name(i).toLowerCase(Locale.US));
            if (!HTTP_2_SKIPPED_REQUEST_HEADERS.contains(m.x())) {
                arrayList.add(new Header(m, headers.value(i)));
            }
        }
        return arrayList;
    }
    ...
    public ResponseBody openResponseBody(Response response) throws IOException {
        ...
        Logger logger = s7.a;
        return new RealResponseBody(header, contentLength, new da(streamFinishingSource));
    }
