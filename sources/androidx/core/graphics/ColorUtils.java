...
public final class ColorUtils {
    ...
    public static void RGBToXYZ(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull double[] dArr) {
        ...
        if (dArr2.length == 3) {
            ...
            dArr2[0] = ((0.1805d * d3) + (0.3576d * d2) + (0.4124d * d)) * XYZ_WHITE_REFERENCE_Y;
            dArr2[1] = ((0.0722d * d3) + (0.7152d * d2) + (0.2126d * d)) * XYZ_WHITE_REFERENCE_Y;
            double d7 = d3 * 0.9505d;
            dArr2[2] = (d7 + (d2 * 0.1192d) + (d * 0.0193d)) * XYZ_WHITE_REFERENCE_Y;
            return;
        }
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3) {
        ...
        double d5;
        double d6;
        double d7 = ((-0.4986d * d3) + ((-1.5372d * d2) + (3.2406d * d))) / XYZ_WHITE_REFERENCE_Y;
        double d8 = ((0.0415d * d3) + ((1.8758d * d2) + (-0.9689d * d))) / XYZ_WHITE_REFERENCE_Y;
        double d9 = ((1.057d * d3) + ((-0.204d * d2) + (0.0557d * d))) / XYZ_WHITE_REFERENCE_Y;
        if (d7 > 0.0031308d) {
    ...
    public static Color compositeColors(@NonNull Color color, @NonNull Color color2) {
        if (Objects.equals(color.getModel(), color2.getModel())) {
            ...
            float[] ab = color.getComponents();
            float[] ab2 = color2.getComponents();
            float a = color.alpha();
            float a2 = (1.0f - a) * color2.alpha();
            int b = color2.getComponentCount() - 1;
            float f = a + a2;
            ab2[b] = f;
            if (f > 0.0f) {
                a /= f;
            ...
            for (int i = 0; i < b; i++) {
                ab2[i] = (ab2[i] * a2) + (ab[i] * a);
            }
            return Color.valueOf(ab2, color2.getColorSpace());
        }
