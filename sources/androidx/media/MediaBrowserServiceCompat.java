...
public abstract class MediaBrowserServiceCompat extends Service {
            ...
            MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() {
                public void run() {
                    ...
                    if (!MediaBrowserServiceImplApi21.this.mRootExtrasList.isEmpty()) {
                        MediaSessionCompat.Token token = token;
                        synchronized (token.c) {
                            bVar = token.g;
                        }
                        if (bVar != null) {
                            for (Bundle putBinder : MediaBrowserServiceImplApi21.this.mRootExtrasList) {
                                BundleCompat.putBinder(putBinder, MediaBrowserProtocol.EXTRA_SESSION_BINDER, bVar.asBinder());
                            }
                        }
                        MediaBrowserServiceImplApi21.this.mRootExtrasList.clear();
                    }
                    MediaBrowserServiceImplApi21.this.mServiceFwk.setSessionToken((MediaSession.Token) token.f);
                }
            });
        }
    ...
    public void performLoadItem(String str, ConnectionRecord connectionRecord, final ResultReceiver resultReceiver) {
        ...
        this.mCurConnection = null;
        if (!r0.isDone()) {
            throw new IllegalStateException(y2.i("onLoadItem must call detach() or sendResult() before returning for id=", str));
        }
    }
    ...
    public void performSearch(String str, Bundle bundle, ConnectionRecord connectionRecord, final ResultReceiver resultReceiver) {
        ...
        this.mCurConnection = null;
        if (!r0.isDone()) {
            throw new IllegalStateException(y2.i("onSearch must call detach() or sendResult() before returning for query=", str));
        }
    }
