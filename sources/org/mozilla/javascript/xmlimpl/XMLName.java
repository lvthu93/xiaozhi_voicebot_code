...
class XMLName extends Ref {
    ...
    private static boolean isNCNameChar(int i) {
        if ((i & -128) == 0) {
            ...
            if (isNCNameStartChar(i) || i == 183 || (768 <= i && i <= 879)) {
                return true;
            }
            return false;
        } else if (isNCNameStartChar(i) || (8255 <= i && i <= 8256)) {
            return true;
        } else {
            return false;
        }
    ...
    public final boolean matches(XML xml) {
        ...
        if (!this.isAttributeName) {
            if (uri() == null || (xml.isElement() && uri().equals(str))) {
                if (localName().equals("*")) {
                    return true;
                }
            ...
            if ((uri() == null || uri().equals(str)) && (localName().equals("*") || localName().equals(nodeQname.getLocalName()))) {
                return true;
            }
            return false;
        }
