...
public class SSLSocketChannel2 implements ByteChannel, WrappedByteChannel, ISSLChannel {
    ...
    private final k4 log = l4.e(SSLSocketChannel2.class);
    ...
    public void createBuffers(SSLSession sSLSession) {
        ...
        if (this.inData.remaining() != 0 && this.log.e()) {
            this.log.i(new String(this.inData.array(), this.inData.position(), this.inData.remaining()));
        }
        this.inData.rewind();
        this.inData.flip();
        if (this.inCrypt.remaining() != 0 && this.log.e()) {
            this.log.i(new String(this.inCrypt.array(), this.inCrypt.position(), this.inCrypt.remaining()));
        }
        this.inCrypt.rewind();
        this.inCrypt.flip();
    ...
    public boolean isNeedRead() {
        if (this.saveCryptData == null && !this.inData.hasRemaining() && (!this.inCrypt.hasRemaining() || this.readEngineResult.getStatus() == SSLEngineResult.Status.BUFFER_UNDERFLOW || this.readEngineResult.getStatus() == SSLEngineResult.Status.CLOSED)) {
            return false;
        }
        return true;
    }
