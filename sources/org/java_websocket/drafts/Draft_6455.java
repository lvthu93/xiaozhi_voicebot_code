...
public class Draft_6455 extends Draft {
    ...
    private void checkBufferLimit() throws LimitExceededException {
        ...
        if (byteBufferListSize > ((long) this.maxFrameSize)) {
            clearBufferList();
            this.log.f("Payload limit reached. Allowed: {} Current: {}", Integer.valueOf(this.maxFrameSize), Long.valueOf(byteBufferListSize));
    ...
    private HandshakeState containsRequestedProtocol(String str) {
        for (IProtocol next : this.knownProtocols) {
            if (next.acceptProvidedProtocol(str)) {
                this.protocol = next;
                this.log.d(next, "acceptHandshake - Matching protocol found: {}");
    ...
    private ByteBuffer createByteBufferFromFramedata(Framedata framedata) {
        ...
        ByteBuffer allocate = ByteBuffer.allocate(payloadData.remaining() + i5 + i2);
        ...
        byte b = (byte) (fromOpcode | ((byte) i3));
        if (framedata.isRSV1()) {
            b = (byte) (b | getRSVByte(1));
        }
        if (framedata.isRSV2()) {
            b = (byte) (b | getRSVByte(2));
        }
        if (framedata.isRSV3()) {
            b = (byte) (getRSVByte(3) | b);
        }
        allocate.put(b);
    ...
    private String generateFinalKey(String str) {
        String x = y2.x(str.trim(), "258EAFA5-E914-47DA-95CA-C5AB0DC85B11");
        try {
            return Base64.encodeBytes(MessageDigest.getInstance("SHA1").digest(x.getBytes()));
    ...
    private void logRuntimeException(WebSocketImpl webSocketImpl, RuntimeException runtimeException) {
        this.log.c("Runtime exception during onWebsocketMessage", runtimeException);
    ...
    private void processFrameContinuousAndNonFin(WebSocketImpl webSocketImpl, Framedata framedata, Opcode opcode) throws InvalidDataException {
        ...
        if (opcode != opcode2) {
            ...
            this.log.a("Protocol error: Continuous frame sequence was not started.");
            throw new InvalidDataException(1002, "Continuous frame sequence was not started.");
        }
        if (opcode == Opcode.TEXT && !Charsetfunctions.isValidUTF8(framedata.getPayloadData())) {
            this.log.a("Protocol error: Payload is not UTF8");
    ...
    private void processFrameIsFin(WebSocketImpl webSocketImpl, Framedata framedata) throws InvalidDataException {
        ...
        this.log.i("Protocol error: Previous continuous frame sequence not completed.");
        throw new InvalidDataException(1002, "Continuous frame sequence was not started.");
    }
    ...
    private Framedata translateSingleFrame(ByteBuffer byteBuffer) throws IncompleteException, InvalidDataException {
        ...
        if (byteBuffer != null) {
            ...
            translateSingleFrameCheckLengthLimit((long) i3);
            if (z5) {
                i = 4;
            } else {
                i = 0;
            }
            translateSingleFrameCheckPacketSize(remaining, i2 + i + i3);
            ...
            this.currentDecodingExtension.isFrameValid(framedataImpl1);
            this.currentDecodingExtension.decodeFrame(framedataImpl1);
            if (this.log.e()) {
                k4 k4Var = this.log;
                ...
                k4Var.f("afterDecoding({}): {}", valueOf, str);
            }
    ...
    private void translateSingleFrameCheckLengthLimit(long j) throws LimitExceededException {
        if (j <= 2147483647L) {
            ...
            if (j > ((long) i)) {
                this.log.f("Payload limit reached. Allowed: {} Current: {}", Integer.valueOf(i), Long.valueOf(j));
                throw new LimitExceededException("Payload limit reached.", this.maxFrameSize);
            } else if (j < 0) {
                this.log.i("Limit underflow: Payloadsize is to little...");
                throw new LimitExceededException("Payloadsize is to little...");
            }
        } else {
            this.log.i("Limit exedeed: Payloadsize is to big...");
    ...
    private void translateSingleFrameCheckPacketSize(int i, int i2) throws IncompleteException {
        if (i < i2) {
            this.log.i("Incomplete frame: maxpacketsize < realpacketsize");
    ...
    private TranslatedPayloadMetaData translateSingleFramePayloadLength(ByteBuffer byteBuffer, Opcode opcode, int i, int i2, int i3) throws InvalidFrameException, IncompleteException, LimitExceededException {
        ...
        if (opcode == Opcode.PING || opcode == Opcode.PONG || opcode == Opcode.CLOSING) {
            this.log.i("Invalid frame: more than 125 octets");
    ...
    public HandshakeState acceptHandshakeAsClient(ClientHandshake clientHandshake, ServerHandshake serverHandshake) throws InvalidHandshakeException {
        if (!basicAccept(serverHandshake)) {
            this.log.i("acceptHandshakeAsClient - Missing/wrong upgrade or connection in handshake.");
            return HandshakeState.NOT_MATCHED;
        } else if (!clientHandshake.hasFieldValue(SEC_WEB_SOCKET_KEY) || !serverHandshake.hasFieldValue(SEC_WEB_SOCKET_ACCEPT)) {
            this.log.i("acceptHandshakeAsClient - Missing Sec-WebSocket-Key or Sec-WebSocket-Accept");
            return HandshakeState.NOT_MATCHED;
        } else {
            if (!generateFinalKey(clientHandshake.getFieldValue(SEC_WEB_SOCKET_KEY)).equals(serverHandshake.getFieldValue(SEC_WEB_SOCKET_ACCEPT))) {
                this.log.i("acceptHandshakeAsClient - Wrong key for Sec-WebSocket-Key.");
                return HandshakeState.NOT_MATCHED;
            }
            ...
            while (true) {
                ...
                if (next.acceptProvidedExtensionAsClient(fieldValue)) {
                    ...
                    this.log.d(next, "acceptHandshakeAsClient - Matching extension found: {}");
                    break;
                }
            }
            ...
            this.log.i("acceptHandshakeAsClient - No matching extension or protocol found.");
            return HandshakeState.NOT_MATCHED;
        }
    }
    ...
    public HandshakeState acceptHandshakeAsServer(ClientHandshake clientHandshake) throws InvalidHandshakeException {
        if (readVersion(clientHandshake) != 13) {
            this.log.i("acceptHandshakeAsServer - Wrong websocket version.");
            return HandshakeState.NOT_MATCHED;
        }
        HandshakeState handshakeState = HandshakeState.NOT_MATCHED;
        ...
        while (true) {
            ...
            if (next.acceptProvidedExtensionAsServer(fieldValue)) {
                this.negotiatedExtension = next;
                handshakeState = HandshakeState.MATCHED;
                this.log.d(next, "acceptHandshakeAsServer - Matching extension found: {}");
                break;
            }
        }
        ...
        this.log.i("acceptHandshakeAsServer - No matching extension or protocol found.");
        return HandshakeState.NOT_MATCHED;
    }
    ...
    public ByteBuffer createBinaryFrame(Framedata framedata) {
        ...
        if (this.log.e()) {
            ...
            k4Var.f("afterEnconding({}): {}", valueOf, str);
        }
    ...
    public void processFrame(WebSocketImpl webSocketImpl, Framedata framedata) throws InvalidDataException {
        ...
        if (opcode == Opcode.CLOSING) {
            ...
            this.log.a("Protocol error: Continuous frame sequence not completed.");
            ...
            this.log.a("non control or continious frame expected");
    ...
    public String toString() {
        ...
        if (getExtension() != null) {
            StringBuilder o = y2.o(draft, " extension: ");
            o.append(getExtension().toString());
            draft = o.toString();
        }
        if (getProtocol() != null) {
            StringBuilder o2 = y2.o(draft, " protocol: ");
            o2.append(getProtocol().toString());
            draft = o2.toString();
        }
        StringBuilder o3 = y2.o(draft, " max frame size: ");
        o3.append(this.maxFrameSize);
        return o3.toString();
    }
    ...
    public Draft_6455(List<IExtension> list, List<IProtocol> list2, int i) {
        this.log = l4.e(Draft_6455.class);
        this.negotiatedExtension = new DefaultExtension();
