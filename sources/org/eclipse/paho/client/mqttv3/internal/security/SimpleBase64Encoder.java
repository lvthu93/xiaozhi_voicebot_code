...
public class SimpleBase64Encoder {
    ...
    public static byte[] decode(String str) {
        ...
        while (true) {
            ...
            for (int i3 = 2; i3 >= 0; i3--) {
                bArr[i2 + i3] = (byte) ((int) (from64 & 255));
                from64 >>= 8;
            }
            i2 += 3;
        }
        if (length == 3) {
            ...
            for (int i4 = 1; i4 >= 0; i4--) {
                bArr[i2 + i4] = (byte) ((int) (from642 & 255));
                from642 >>= 8;
            }
        }
        if (length == 2) {
            bArr[i2] = (byte) ((int) (from64(bytes, i, 2) & 255));
        }
        return bArr;
    }
    ...
    public static String encode(byte[] bArr) {
        ...
        int i = 0;
        while (length >= 3) {
            stringBuffer.append(to64((long) (((bArr[i] & 255) << 16) | ((bArr[i + 1] & 255) << 8) | (bArr[i + 2] & 255)), 4));
            i += 3;
            length -= 3;
        }
        if (length == 2) {
            stringBuffer.append(to64((long) (((bArr[i] & 255) << 8) | (bArr[i + 1] & 255)), 3));
        }
        if (length == 1) {
            stringBuffer.append(to64((long) (bArr[i] & 255), 2));
        }
    ...
    private static final long from64(byte[] bArr, int i, int i2) {
        ...
        while (i2 > 0) {
            ...
            if (b >= 65 && b <= 90) {
                j2 = (long) ((b + MqttWireMessage.MESSAGE_TYPE_PINGREQ) - 65);
            }
            if (b >= 97 && b <= 122) {
                j2 = (long) ((b + 38) - 97);
            }
